...
@interface TEEventDetailViewController ()

@end

@implementation TEEventDetailViewController
@synthesize eventID;
@synthesize clientIP;
@synthesize type;
...
- (void)loadpicURL{
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"map_personaldetails" ofType:@"html"];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    NSString *param = [NSString stringWithFormat:@"window.initPageManagerClient(%i,320,480,%@,'%@','mappersonaldetails','%@')",self.eventID,uid,pid,type];
    NSString* currentURL = webView.request.URL.absoluteString;
    NSLog(@"currentURL is %@",currentURL);
    if([currentURL hasSuffix:@"com_index.html"]){
        param = [NSString stringWithFormat:@"window.initPageManager(%@,'%@',320,568)",uid,pid];
    }else if([currentURL hasSuffix:@"Trafficeye_new.app/"]){
        param = [NSString stringWithFormat:@"window.initPageManagerClient(%i,320,480,%@,'%@','mappersonaldetails','%@')",self.eventID,uid,pid,type];
    }else if([currentURL hasSuffix:@"map_personaldetails.html"]){
        param = [NSString stringWithFormat:@"window.initPageManagerClient(%i,320,480,%@,'%@','mappersonaldetails','%@')",self.eventID,uid,pid,type];
    }else{
        param = @"";
    }
...
- (void)bingdingStep1:(NSString*)requestType{
    ...
    if([requestType isEqualToString:@"sinaweibo"]){
        type_platform = ShareTypeSinaWeibo;
    }else if([requestType isEqualToString:@"qqweibo"]){
        type_platform = ShareTypeTencentWeibo;
    }
    if([ShareSDK hasAuthorizedWithType:type_platform]){
        //得到所有参数访问接口
        [self bingdingStep2:type_platform :requestType];
    }else{//先授权
        [ShareSDK authWithType:type_platform                                                                     options:nil
...
- (void)bingdingStep2:(int)type_platform :(NSString*)requestType{
    NSString* uid = [[TEAppDelegate getApplicationDelegate] getUid];
    NSString* token = @"";
    NSString* openid = @"";
    id<ISSPlatformCredential> credentialInfo = [ShareSDK getCredentialWithType:type_platform];
    if(credentialInfo){
        NSLog(@"不为nil");
        token = [credentialInfo token];
        openid = [credentialInfo uid];
