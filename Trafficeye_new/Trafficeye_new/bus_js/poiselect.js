(function() {
	...
	window.callbackChooseLocation = function(type, lon, lat, addr) {
		...
		os.set("busEndData", global.jsonUtil.toJsonStr(busEndData));
		pu.toPage("bus_index.html");
	};
	...
	window.setSite = function (resultDataIndex, resultDataIndexIndex, flag) {
		if (isSetSite) {
			...
			var jsonStr = global.jsonUtil.toJsonStr(busEndData);
			os.set("busEndData", jsonStr);
			pu.toPage("bus_index.html");
			return false;
		} else {
			isSetSite = true;
			...
			//调用本地接口
			if (global.mobilePlatform.android) {
				window.JSAndroidBridge.displayPointInMap(lonLat[0], lonLat[1], data.name, data.address, 1);
			} else if (global.mobilePlatform.iPhone) {
				window.location.href="objc:??displayPointInMap::?"+lonLat[0]+":?"+lonLat[1]+":?"+data.name+":?"+data.address+":?"+1;
			} else {
				alert("调用本地invation方法,PC不支持.");
			}
		}
