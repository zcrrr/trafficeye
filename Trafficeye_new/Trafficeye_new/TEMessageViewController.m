...
@interface TEMessageViewController ()

@end

@implementation TEMessageViewController
@synthesize clientIP;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    NSString *param = [NSString stringWithFormat:@"window.initPageManager(%@,%@,0,10,true,'%@')",uid,uid,pid];
    NSString* currentURL = webView.request.URL.absoluteString;
    NSLog(@"currentURL is %@",currentURL);
    if([currentURL hasSuffix:@"Trafficeye_new.app/"]||[currentURL hasSuffix:@"/message.html"]){
        param = [NSString stringWithFormat:@"window.initPageManager(%@,%@,0,10,true,'%@')",uid,uid,pid];
    }else if([currentURL hasSuffix:@"com_index.html"]){
