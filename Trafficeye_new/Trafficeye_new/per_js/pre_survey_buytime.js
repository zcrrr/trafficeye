...
//                                console.log(dataReward);
    ...
    $(function(){
        ...
        pm.init();
                //判断缓存中是否有userinfo信息
        if(myInfo.userinfo){
            var survey_buyTime = Trafficeye.offlineStore.get("survey_buyTime");
            // if(survey_buyTime){
            //     var dataStr = Trafficeye.str2Json(survey_buyTime);
            //     document.getElementById("inputValue").value=dataStr.buyTime;
            // }
            Trafficeye.offlineStore.set("traffic_infosurveycar","survey");
        }else{
            //让用户重新登录
